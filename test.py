...
from predict import tag_predict
...
def main(output_to_file, models):
    if output_to_file:
        print('正在生成测试结果...')
    ...
    if 'hmm' in models or 'vote4' in models:
        print("加载并评估hmm模型...")
    ...
    if 'crf' in models or 'vote4' in models:
        # 加载并评估CRF模型
        print("加载并评估crf模型...")
    ...
    if 'bilstm' in models or 'vote4' in models:
        # bilstm模型
        print("加载并评估bilstm模型...")
    ...
    if 'bilstm+crf' in models or 'vote4' in models:
    ...
    if 'vote+dict' in models:
        print("加载并评估dictionary+vote模型...")
        test_word_lists, test_tag_lists = build_corpus("test", make_vocab=False)
        dict_pred = [tag_predict(''.join(test_word_list)) for test_word_list in test_word_lists]
    
        metrics = Metrics(test_tag_lists, dict_pred, remove_O=REMOVE_O)
        metrics.report_scores()
        metrics.report_confusion_matrix()
...
if __name__ == "__main__":
    output = False
    test_models = ['hmm', 'crf', 'bilstm', 'bilstm_crf', 'vote4', 'vote+dict']
    main(output, test_models)
