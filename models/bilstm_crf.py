...
class BILSTM_Model(object):
    def __init__(self, vocab_size, out_size, word2id, tag2id, crf=True):
        """功能：对LSTM的模型进行训练与测试
           参数:
            vocab_size:词典大小
            out_size:标注种类
    ...
    def train_step(self, batch_sents, batch_tags):
        ...
        loss = self.cal_loss_func(scores, targets, self.tag2id).to(self.device)
        loss.backward()
        self.optimizer.step()
        ...
        with torch.no_grad():
            ...
            for ind in range(0, len(dev_word_lists), self.batch_size):
                ...
                loss = self.cal_loss_func(
                    scores, targets, self.tag2id).to(self.device)
